...
def handle_option(input, client_name, server_socket, ft_listening_port):
    if input in ('M', 'm'):
...
def usage(script_name):
    print('[ ERROR ] Usage: python3 ChatClient.py -l <listening port number> -p <connect server port>')
...
def main():
    ...
    while True:
        ...
        if not input:
            print('[ ERROR ] input could not be read from stdin. exiting.')
            sys.exit()
        handle_option(input, client_name, server_socket, ft_listening_port)
