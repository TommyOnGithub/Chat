...
class TransferManager( threading.Thread ):
    ...
    def recv_file(self, file_host):
        ...
        recv_file = open(self.filename, 'wb')           # Save the received file in the local directory
    ...
    def run(self):
        # print('TM is running at ' + str(time.time()))
        ...
        try:
            ...
            print('[ ERROR ] Unable to bind to given socket ' + str(type(self.ft_listening_port)) + ' ' + str(err))
        ft_listening_socket.listen(5)
        # print('TM socket is listening at ' + str(time.time()))
        while True:
            file_host, addr = ft_listening_socket.accept()
